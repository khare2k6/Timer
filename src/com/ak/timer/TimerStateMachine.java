...
public class TimerStateMachine implements ITimer,IPlaybackListener{
	private IState mCurrentState;
	private long mTime,mLeftTime,mRestPeriodTime ;
	private final String TAG = TimerStateMachine.class.getSimpleName();
	private MyCountDownTimer2 mTimer,mRestPeriodTimer;
	...
	//Constructor
	public TimerStateMachine(Context context){
		mContext = context;
	...
	private class RestPeriodState extends BaseState{
		...
		public void onEntry() {
			...
			mRestPeriodTimer = MyCountDownTimer2.getInstance();
			mRestPeriodTimer.setTimerParams(mContext,
					mRestPeriodTime, INTERVAL, TimerStateMachine.this);
	...
	private class StartedState extends BaseState{
		...
		@Override
		public void onEntry() {
			Log.d(TAG, "starting CountDownTimer for " + mLeftTime / 1000
					+ " sec");
			mEnumState = State.STARTED;
			mTimer = MyCountDownTimer2.getInstance();
			mTimer.setTimerParams(mContext, mTime, INTERVAL, TimerStateMachine.this);
			mTimer.start();
		}
